...
import { validateCompanyEmail } from "@/lib/email-policy";
...
export function SignUpForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      // Server-side trigger also enforces the denylist — relay its message cleanly
      const msg = error.message || "Unable to create account";
      setError(msg.toLowerCase().includes("company email") ? msg : msg);
      setLoading(false);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-gray-500">
          Must be a company email — Gmail, Yahoo, Outlook, iCloud, Proton, etc. are not permitted.
        </p>
      </div>
      <div>
