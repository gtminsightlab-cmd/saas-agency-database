...
**Deprecated:** `C:\Users\GTMin\OneDrive\Documents\Codex\Projects\Saas Agency Database\`. The `.git` is permanently broken (OneDrive sync corrupts git internals). Kept for the vendor `data/` archive only — do NOT edit code there. Future Codex sessions should be launched from the canonical folder.
