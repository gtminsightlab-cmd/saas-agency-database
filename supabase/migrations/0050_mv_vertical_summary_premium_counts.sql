-- Extend mv_vertical_summary with the per-vertical "premium detail" counts
-- surfaced on the verticals page: contacts_with_email, contacts_with_mobile,
...
SELECT
  ...
  COALESCE((SELECT count(*) FROM agency_match WHERE vertical_id = vm.id AND matches >= 4), 0)::bigint AS agencies_growing,
  COALESCE((SELECT count(*) FROM agency_match WHERE vertical_id = vm.id AND matches >= 7), 0)::bigint AS agencies_specialist,
