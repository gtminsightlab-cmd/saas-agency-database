...
**Last refresh:** 2026-05-19 (seven16-group-site Session 3 close — Support-platform integration readiness SHIPPED at `partners.seven16group.com` (commits `64e3cf0` + `e591b8d`, Vercel deploy READY). 4 internal API stubs at `/api/internal/support/{health,context,events,ticket-link}` (health 200 real; others 501 stubs, all bearer-gated). 4 `src/lib/support/*` helpers (config + context + events + auth w/ `crypto.timingSafeEqual`). Widget placeholder mounted once on marketing layout; renders null while `SUPPORT_PLATFORM_API_KEY` holds the `PLACEHOLDER_REPLACE_ME` placeholder. 6 new env vars in Vercel production (3 non-secret defaults + 3 sensitive placeholders). Architectural rule LOCKED: support-INTEGRATABLE not support-DEPENDENT.<br>Prior 2026-05-19 SESSION_27 close — Internal-app foundation shell + Recruit Lists wrap SHIPPED at directory.seven16group.com (commit `0a8b2dc`, deploy `dpl_8SRxpyK5q3kwXVBnUzpmM5zdBn8s` READY, 42s build). First slice of Sessions 27-32 internal-app redesign epic; 5 of 6 sessions remaining. Tailwind palette swap (brand-* → #0F766E teal-700 anchor + intel-cyan + intel-dark) visually re-tinted 256 occurrences across 40 files. 5 new `components/app/*` primitives (PageHeader, Breadcrumbs, TopBar, MetricCard, DataTable). AppShell refactored + Sidebar relabeled + Recruit Lists page rewritten + row-actions D-024 polish.)
