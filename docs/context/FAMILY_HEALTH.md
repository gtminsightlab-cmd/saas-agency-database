...
**Last refresh:** 2026-05-19 (SESSION_27 close — Internal-app foundation shell + Recruit Lists wrap SHIPPED at directory.seven16group.com (commit `0a8b2dc`, deploy `dpl_8SRxpyK5q3kwXVBnUzpmM5zdBn8s` READY, 42s build). First slice of Sessions 27-32 internal-app redesign epic; 5 of 6 sessions remaining. Tailwind palette swap (brand-* → #0F766E teal-700 anchor + intel-cyan + intel-dark) visually re-tinted 256 occurrences across 40 files. 5 new `components/app/*` primitives (PageHeader, Breadcrumbs, TopBar, MetricCard, DataTable). AppShell refactored + Sidebar relabeled + Recruit Lists page rewritten + row-actions D-024 polish. 10 files / 591 insertions / 245 deletions. Live verified end-to-end (delete + toast + KPI recompute). Side finding: 3 pre-existing GitHub Dependabot vulnerabilities flagged on push (1 high, 2 moderate; queued for triage).<br>Prior 2026-05-18 SESSION_26 close — Path A marketing homepage redesign SHIPPED at directory.seven16group.com (commits `b8defba` + `f24b3f3`, 25 files / +1939/-1334). First major application of D-024: 6 shared `components/ui/*` primitives + 13 marketing section components + sonner Toaster + jsx-a11y flat-config lint.)
...
|---|---|---|---|---:|---:|:-:|
| **saas-agency-database** | 2026-05-19 `0a8b2dc` (1 commit this session, deploy READY 42s) | 0 (SESSION_27 close) | (AS) Internal-app foundation shell + Recruit Lists wrap SHIPPED. First slice of Sessions 27-32 epic (1 of 6 done). Palette swap re-tinted 256 brand-* refs to #0F766E. 5 new `components/app/*` primitives. Sidebar relabeled. Recruit Lists page has KPI row + DataTable + StatusPill + sonner toasts. Next: SESSION_28 — Intelligence Home + Vertical Intelligence redesign (paste-ready at `SESSION_28_PROMPT.md`). | 0 (AS) | 11 + 6 carry + 5 epic-sessions remaining | 🟢 |
...
- **`[ACTION] D-024 apply-on-touch backlog`** — pre-existing jsx-a11y errors surfaced by the SESSION_26 flat-config wire-up that need cleanup on next touch: `app/sign-up/form.tsx` (2× anchor-is-valid + 1× react-hooks/exhaustive-deps missing plugin), `app/global-error.tsx` (html-has-lang). Not session-26-introduced; clean on next edit per D-024 apply-on-touch policy.
...
- **`[FAMILY DOCTRINE - LOCKED]` Seven16 Group Partner Program integration** — locked 2026-05-18 by Master O at SESSION_26 close. Parent hub at `partners.seven16group.com` (NOT built yet). **DO NOT build standalone referral/affiliate/partner/commission systems inside any product.** Per-product PRODUCT_KEY: `agency_signal` / `dotintel` / `dotagencies`. Full doctrine at family memory [`project_seven16_partner_program.md`](C:\Users\GTMin\.claude\projects\C--Users-GTMin-Projects-saas-agency-database\memory\project_seven16_partner_program.md). Surface in every session opening that touches referrals / affiliates / partners / commissions / coupon-as-reward / sign-up attribution. **Cross-repo:** DotIntel and DotAgencies session prompts need a similar STEP 2.5 standing reminder block added when their next-session prompts get written (use their respective PRODUCT_KEYs).
