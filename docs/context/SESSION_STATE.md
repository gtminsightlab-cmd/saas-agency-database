...
**Last updated:** 2026-05-23 (**Dashboard actions arc CLOSED — Session 33 (`docs/handoffs/SESSION_33_HANDOFF.md`).** 2 of 3 done cleanly + 1 deferred to ~5-min fresh-eyes follow-up next session. **Action 1 ✅:** CRON_SECRET generated via browser DevTools console fallback (pre-staged PowerShell command failed on Windows PowerShell 5.1 / .NET Framework 4.x — `[RandomNumberGenerator]::Fill()` is .NET 6+ only; clean cross-platform fallback documented in handoff) + set in Vercel (Production+Preview, Sensitive). First proof point lands on next 04:00 UTC cron run (saved_list_snapshots currently 0 rows). **Action 2 ✅ (no action needed):** Stripe webhook discovered already wired (`Seven16-Agency-Directory` endpoint Active, 18 events, 0% error rate, delivery activity proven via Workbench) — BACKLOG / pending-actions doc was stale on this front; `STRIPE_WEBHOOK_SECRET` proven in Vercel via 200-response inference (webhook handler returns 400 on missing secret). **Action 3 ⚠️ (deferred):** Sentry token minted but production build STILL shows `[@sentry/nextjs - After Production Compile] Warning: No auth token provided` across all 8 redeploys this session — non-blocking (site READY, customers unaffected); pickup task = filter Vercel env vars by Production scope, verify all 3 of SENTRY_AUTH_TOKEN+SENTRY_ORG+SENTRY_PROJECT exist there with non-empty values, redeploy once. Production HEAD `708d3fd` with Vercel deploy READY on `dpl_6PpfA5ye4CjyyNZTkjBQ2NnJRReq` (8th of 8 redeploys). Pending-actions doc stamped SUPERSEDED. Prior 2026-05-22 (**3-PR merge sweep complete — PR #4 + PR #1 + PR #5 all merged to main.** Production HEAD `f7618aa` (qs squash merge from PR #5); prior `363bd78` (ws merge from PR #1); prior `a8b9c1e` (BACKLOG #1 /home v1 + epic cleanup from PR #4). All Vercel deploys READY. Dependabot vulnerability count: 3 → 2 (ws resolved; xlsx + next/postcss remain as documented-ACCEPT per `docs/security/dependabot-triage-2026-05-22.md`). Lint at 0 errors (epic-level 42 → 0, -100%). **3 dashboard actions still pending for Master O at laptop (~10 min total):** CRON_SECRET + Stripe webhook + Sentry token rotation. Full checklist with PowerShell commands + exact URLs at `docs/handoffs/MASTER_O_PENDING_DASHBOARD_ACTIONS_2026-05-22.md`. Cannot be Claude-automated due to secrets-never-in-chat standing rule + no env-var MCP + family doctrine. Prior 2026-05-22 (**BACKLOG #1 SHIPPED — /home personalization v1 + post-login redirect flip.** First commit after Sessions 27-32 epic close. 2 files / +326/-58 — ~1.5 hrs vs ~2-3 hr original estimate; 6th consecutive scope-under-prompt delivery. Smart-degradation design ships independent of cron snapshot data (counters render 0 today; auto-light-up when CRON_SECRET is set + cron fires). Commit `ead0b71`. PR #4 draft open with Vercel preview READY. **Open dashboard action (Master O):** set CRON_SECRET in Vercel production env (~3 min). After that, BACKLOG #3 moves to Done as verification-only. Prior 2026-05-22 (**Sessions 27-32 internal-app redesign epic CLOSED end-to-end.** Session 32 shipped slice 6 of 6 — admin polish via lint sweep + stale-copy fix + dead-code cleanup. Path A descope #3 on admin chrome (intentional dark-themed `admin-*` token design already at production polish; AdminSidebar + AdminTopbar + Master Control Room + System Health + Integrations + Billing pre-existed). 1.5 hrs / 7 files / +88/-124 — 85% under SESSION_H_PROMPT estimate. Lint progression this session: 11 → 1 (-91%); only remaining = pre-existing `analytics/carriers/page.tsx:59` directive in untouched file. Build ✓ 14.7s. **Epic totals:** 6 sessions × ~14 hrs wall (vs original ~30-40 hr estimate; ~60% under). Lint across epic: 42 → 1 (-97%). 3 Path A descopes locked. Recovery branch `claude/session-30-recovery-D4xSt` HEAD `e4e2c36` carrying SESSION_31 memory refresh + SESSION_32 polish on PR #3 awaiting Master O merge approval. Active arc now flips back to BACKLOG — Master O picks next (BACKLOG #1 /home personalization v1, #2 Quick Search 4-tab build, or pivot to first-paying-customer Stripe cutover). Prior 2026-05-22 (**Session 31 close — Sessions 27-32 epic 5 of 6 SHIPPED.** Recovery branch `claude/session-30-recovery-D4xSt` opened after the parent Session 30 conversation went silent before close-out — three commits shipped (`4fcaba4` missing Session 30 handoff recovery + `597520b` /data-stats + /team chrome polish + `b53b034` Session 31 close-out docs) and merged to `main` via PR #2 (merge commit `70c0fbe`). Big audit finding: /methodology + /resources are full marketing surfaces with hero designs, NOT bare authed pages — Path A descope same pattern Session D locked for /verticals. /data-stats renamed visible title "Data and Stats" → "Data Coverage" matching the SESSION_27 sidebar (stale-label catch like /ai-support in Session 30). /team removed anti-slop `← Back` link + dropped dead `owner` variable. Lint stayed at 11 (no new; pre-existing analytics/carriers:59 directive queued). Build ✓ 14.0s. Vercel preview READY on `597520b`. Audit-first now non-negotiable — 4 sessions in a row delivered 30-80% under prompt estimates. New main HEAD `70c0fbe`. Next: SESSION_32 (Admin polish — AdminSidebar dark + Master Control Room + System Health + Billing + Integrations, final epic slice) per `SESSION_H_PROMPT.md`. Prior 2026-05-20 (**Session 30 close — Sessions 27-32 epic 4 of 6 SHIPPED.** Tonight's outing across one conversation: Session B-close Texas DOI 367k row load → Session C Seven16 Group Support readiness for AS → Session D `/home` dashboard + `/verticals/[slug]` polish (Path A audit divergence; preserved /verticals marketing surface) → Flag 2 admin/verticals manager.tsx React 19 refactor → Session 29 Build Recruit List chrome + lint sweep → Session 30 Exports/Agency Search/AI Research Assistant chrome. 5 production deploys verified READY. Lint 42 → 11 (-31, 74% reduction). Audit-first now load-bearing — Session 28: -30%, Session 29: -30%, Session 30: -65% vs prompt estimates. New BACKLOG #2 queue item surfaced: Quick Search 4-tab feature build (~6-8 hrs, separate dedicated session — current `/quick-search` is placeholder form). Latest commit `b5519a4` (Session 30); deploy `dpl_tybMUW98RXKK7YLkiFyeUSasvkH1` READY at directory.seven16group.com. Next: SESSION_31 (Data Coverage + Methodology + Resources + Team) per `SESSION_G_PROMPT.md` — ~5 hr ceiling per CMO brief 2 but audit-first pattern says treat as ceiling. Prior 2026-05-19 — Session B Texas 2026 DOI appointment load LIVE end-to-end:** `public.agency_carriers` 263,657 → 631,114 (+367,457 TX rows tagged `source_type='state_doi_tx'`); `public.agencies` 41,705 → 58,490 (+16,785 new TX); `public.carriers` 1,369 → 2,007 (+638). 27/367k rejected (0.007%). Prior 2026-05-19 (**Session A close — state DOI appointment ETL schema prep shipped.** Migrations 0093 + 0094 applied to `sdlsdovuljuymgymarou`. `agencies` gains `npn` + `ein` (non-unique B-tree indexes). `agency_carriers` gains `appointment_active_date` + `state_filed` + `source_year` + composite `(agency_id, carrier_id, state_filed)` index. New `ax_staging` schema with 4 tables (`appointment_loads` / `appointment_load_pages` / `appointments_raw` / `appointment_rows_rejected`) — RLS forced, service-role only, NOT exposed via PostgREST. `source_type` taxonomy locked at `docs/context/SOURCE_TYPE_TAXONOMY.md`. D-025 logged. NAIC mapping CSV generated for Master O spot-check at `tmp/texas_naic_mapping.csv` (937 distinct carriers in Texas 2026 file → 300 high-confidence map / 301 review / 336 net-new). Zero new advisor findings. Internal-app redesign epic SESSION_28 deferred until Texas DOI load (Session B) ships — data work > UI work for product moat.) Prior 2026-05-19 (SESSION_27 close — internal-app foundation shell + Recruit Lists wrap shipped. First slice of Sessions 27-32 internal-app redesign epic. Tailwind palette swapped brand-* → #0F766E teal-700 anchor. 5 new components/app/* primitives. Sidebar relabeled. Recruit Lists page rewritten with 5-KPI row + DataTable wrapper + StatusPill + sonner toasts.)
**Companion to:** [MASTER_CONTEXT.md](MASTER_CONTEXT.md)

> Snapshot of where each product stands **right now**. Three platform products in the family — Seven16 Intel (live), DOT Intel (demo build at dotintel.io), Threshold IQ (build in progress in another session) — plus standalone-capable add-ons (Growtheon reseller, Seven16Recruit stealth) and parked future products. Read the relevant section before starting work.

### Quick-ref: where each repo lives + latest commit

| Repo | Canonical clone | Latest commit on `main` |
|---|---|---|
| `saas-agency-database` (family hub + Seven16 Intel) | `C:\Users\GTMin\Projects\saas-agency-database\` | `f7618aa` — qs@6.15.2 squash merge from PR #5 (transitive dep bump). Prior on main: `363bd78` ws@8.20.1 merge from PR #1; `2959243` family-health doc bump from seven16-group-site Session 5 close; `a8b9c1e` BACKLOG #1 /home v1 + epic cleanup merge from PR #4. All Vercel production deploys verified READY. Live URL: https://directory.seven16group.com. |
...
- **D-015:** Enterprise+ pricing for the second ICP (Distribution Expanders). Seven16 Intel state-based slider on **verified email contacts**: 5 Tier 1 ($2,000), 15 Tier 2 ($1,500), 28 Tier 3 ($1,000), 3 territorial add-ons ($0). All-50 ceiling $12,500 = 50% undercut of Neilson's $25k. Distribution+ outcome SKU $300–$500/qualified appointment. Distribution Suite bundle $22,500/yr.
- **Data refresh:** Live Seven16 Intel database now has 41,705 agencies / 263,657 carrier appointments / 1,085 active writing companies / 135,453 contacts (85.6% with verified email).
...
- `agency_signal` — Seven16 Intel — seven16intel.com — active
...
**Seeded plans (9 total — locked pricing only):** Seven16 Intel Free/Producer($19)/Growth($99)/Enterprise + DOT Intel Free/Pro($29)/Business($149)/Enterprise + Seven16 Intelligence Bundle ($179, available_from 2027-01-01). **Pending:** Threshold IQ, Growtheon, Seven16Recruit pricing — collaborative session needed before seeding.

**Future Supabase satellites per D-008** (not yet created):
- `seven16-agency-signal` — repurpose existing `sdlsdovuljuymgymarou` (currently Seven16 Intel's prod DB)
...
**Next sprints:**
- Sprint 1C: shared JWT secret across `seven16-platform` ↔ `sdlsdovuljuymgymarou` (Seven16 Intel) + `yyuchyzmzzwbfoovsskz` (Threshold IQ — joined client list 2026-05-02 when staging went live); Doppler + Sentry rollout. NEEDS Vercel API token + Master O time on Doppler signup.
- Sprint 1D (planned window): migrate `auth.users` from Seven16 Intel → platform; rewire app reads via shared JWT. Threshold IQ folds in same sprint (or follows immediately).
...
- `saas-agency-database/docs/STATE.md` — Seven16 Intel inside view (queued; lower priority since family ledger lives in this same repo)
...
## Part 1 — Seven16 Intel (live, now in its own dedicated session as of 2026-05-07)
...
### 1.5 Deferred items (Seven16 Intel)
...
> **S59 update (2026-05-29) — Trucking Underwriting Submission Intake module SHIPPED + pushed** (`dotintel2` main `64092c8`, auto-deploying to Vercel). New `submissions` dashboard module (operator category): ACORD-aligned parent account (`tenant_submissions`) + Driver/Power Unit/Trailer schedules. Two entry modes — metadata-driven tabbed editor AND Excel template/import (validate→commit, parent rollback on child error). USDOT pre-fills legal name/garaging/fleet counts from `public.carriers` corpus via `get_submission_carrier_prefill` (SECURITY INVOKER, RLS-respecting). 5 migrations `20260619`→`20260623` applied live to `vbhlacdrcqdqnvftqtin`; advisors held at baseline (security 75 INFO34/WARN41; performance WARN 51); all tables RLS forced + per-command policies. New server-only dep `exceljs` (MIT; 1 moderate Dependabot alert on transitive — re-audit next bump). **Migration collision:** Submission **Desk** Phase 5 reserved `20260619`→`20260622` — must renumber to ≥`20260624`. **Compliance gate:** driver PII (DOB/license#) needs legal review before any external tester (hard rule 13). Monetization Phase 3 (Stripe) still the default next theme. Spec: `dotintel2/docs/specs/dotintel-submission-intake-spec.md`. Next-session plan: `dotintel2/docs/SESSION_60_PROMPT.md`.

> **S58 update (2026-05-29) — Monetization Phase 1 + Phase 2 APPLIED LIVE on `vbhlacdrcqdqnvftqtin`.** Migrations `20260608`→`20260618`: plan catalog + per-tier entitlement matrix (free/starter $49/producer_pro $99/niche_builder $199/agency_team $399/growth_agency $799/enterprise + credit-pack add-ons) + tenant auto-provisioning (1/user, keyed off `auth.users`; 5 demo users backfilled to `free`) + full entitlement function set (`get_entitlement`/`consume_entitlement` etc.) + `monitoring_dot` resource-cap BEFORE-INSERT trigger + `tenant_renewal_calendar` view + `tenant_pipeline_items` CRM table. **Tenant is now the single source of truth for plan/entitlements** (extends, does not replace, the existing `tenants`/`tenant_members`/`dotintel_entitlements`). Advisors security 75 / performance 214 — no new ERROR/blocker. PENDING: Phase 3 Stripe (test mode) → Phase 4 retrofit of live call sites → Phase 5 Submission Desk. Next-session plan: `dotintel2/docs/SESSION_59_PROMPT.md`. Locked decisions: family memory `project_dotintel_monetization_arch.md`.
...
**Sprint D0 — Color match.** Tailwind v4 `@theme inline` block in `dotintel2/app/globals.css` aligned to Seven16 Intel anchors (brand-600 teal, gold-500, navy-900). Existing semantic class names preserved. Commit `91bf52b`.
...
**Persona quick-fill gated off public DOT Intel login (2026-05-04).** User asked to remove the four persona quick-fill buttons (Agent / Underwriter / Risk Mgr / Analyst) from `dotintel.io/login` so credentials aren't visible to outsiders pre-demo. Gated the demo block behind a top-of-file constant `SHOW_DEMO_QUICK_FILL` defaulting to `false` in `components/marketing/login-form.tsx` (`2946e3f`). Auth users in `vbhlacdrcqdqnvftqtin` are unchanged and continue to work via manual entry. Flip the constant to `true` and push to re-enable for the mid-May working-group demo. **Hygiene flag:** demo passwords are committed in plaintext in this file. Repo is private but plaintext-in-git is a smell; rotate post-demo and either move array to a server-only config + cookie pre-fill or delete entirely. Logged as 🟠 post-demo MUST-DO in the handoff known-issues.

**Session 19 — Cloudflare Email Routing + Coverage Amounts ETL + Premium Estimator RPC.** `info@dotintel.io` → `gtminsightlab@gmail.com` live (Cloudflare Email Routing free tier, DNS auto-added, test confirmed). Coverage amounts ETL: 18,090 / 19,767 rows in `carrier_insurance_current` backfilled with `min_cov_amount` / `max_cov_amount` from `inshist_raw.raw_row` JSONB (values ×1000 for thousands-to-dollars conversion). Migration `20260507_backfill_coverage_amounts`. Also applied `get_premium_estimate_comparables` RPC to Supabase, built the three Premium Estimator route files (layout, page, form component). Cloudflare MCP configured in `~/.claude/settings.json`. BSB policy number patterns reference memory seeded.

**Session 20 — Premium Estimator shipped to production.** Dashboard module card updated from `coming_soon` → `active` with `href: "/dashboard/premium-estimator"`. Migration file `20260508_premium_estimator_comparables_rpc.sql` synced to repo. Commit `6431c8d` pushed and deployed — Vercel READY in ~35s. Full module now live at `dotintel.io/dashboard/premium-estimator`: 5-panel input form (PU count, state, haul type, authority age, safety rating, coverage target), rules-based estimate engine (base rate by PU band × state/coverage/safety/haul/authority multipliers), Similar Carriers panel from live corpus, localStorage persistence for form inputs.

**Sessions 21–24 (2026-05-10 — DOT Intel only).** Multi-session day spent inside dotintel2 — full inside-view detail in `dotintel2/docs/STATE.md`. Family-level summary: Territory Intelligence rename + Seven16 Intel cross-sell, Underwriter / Appetite Intelligence naming alignment + Premium Estimator and Underwriter Intelligence solution rows, DOT Lookup standalone hero + open `/signup` flow with email gate fully relaxed (server trigger dropped — drivers / fleet owners / personal-email users can now self-signup), TOTP MFA + backup codes shipped at `/account/security`, two-tier CLAUDE.md adoption (family-level `ENGINEERING_DOCTRINE.md` added to this `docs/context/` directory), v1 risk-scoring upgrade (public `/methodology` page + hard disqualifier override → Ineligible band + 24-month linear decay helper + three new historical-insurance factors from `inshist_raw`). Risk-scoring spec + status tracker archived at `dotintel2/docs/specs/dotintel-risk-scoring-{spec,kickoff,status}.md` — Task F (`risk_score_history` table + 90-day trend strip) deferred to ~2026-06-15 with architectural decisions pre-locked in family memory.

**Session 25 — Directory build kickoff (2026-05-10).** Three commits, three deliverables. (1) Directory build spec + kickoff + status tracker archived at `dotintel2/docs/specs/dotintel-directory-build-{spec,kickoff,status}.md` — engineering contract for the five new schemas + six-phase build sequence. (2) 32 Stitch UI inspiration mocks (HTML+PNG per surface) archived at `dotintel2/docs/specs/dotintel-directory-build-inspiration/`. (3) Task 1.B from Phase 1 shipped — `content` schema migration applied to `vbhlacdrcqdqnvftqtin` (four tables: `content.sources`, `content.news_raw`, `content.news_classified`, `content.posts` per spec §4.4); RLS forced on every table; Supabase advisors clean (0 WARN+ findings). HEAD on `origin/main` rotated to `289027a`; Vercel production deploy `dpl_Efm8pJfshem3f1qY6ppXYRbgXzSq` READY. **D-012 logged 2026-05-10:** the directory build host is dotintel2 + vbhlacdrcqdqnvftqtin (not the saas-agency-database satellite the spec literally diagrammed). Reason: D-008 satellite separation; dotintel2 owns the heavier data spine; agency-side data mirrors over via future Task 1.F. Phase 1 task sequence + acceptance criteria in `dotintel2/docs/specs/dotintel-directory-build-status.md` — that's the live tracker for this stream of work.

**Late-session scrapers fragment-bug investigation.** Cross-session flag from another session: "entries like 'Agency LLC', 'Group LLC' with no state/ZIP showing up cross-source. That's parse_berkley_pastes.py capturing trailing fragments." Investigation: **both production databases are clean** (Seven16 Intel `sdlsdovuljuymgymarou` agencies 20,739 rows / DOT Intel `vbhlacdrcqdqnvftqtin` agencies 53 rows — zero fragment matches in either). The loader at `scripts/load_to_supabase.py:200` has a `if not (state and zip_code): continue` gate that kept the bug out of prod. Diagnosis was misdirected — `parse_berkley_pastes.py` already has its own state/zip guard. Actual culprit: `scripts/run_trustedchoice.py:106` `harvest_state()` regex `\b([A-Z][A-Za-z0-9.&\-' ]{2,80}?\s+(?:Insurance|Agency|...|LLC|Inc\.))\b` lazy-quantifier captures standalone footer-link / category text. Cross-source JSONL scan: 151 fragment rows in trustedchoice files (42 'Agency LLC', 41 'Services LLC', 35 'Group LLC', 22 'Associates LLC', 9 'Insurance LLC', 2 'Enterprises LLC'). Patched `run_trustedchoice.py` with a `SUFFIX_ONLY` post-filter rejecting names whose firm-name portion is itself a generic suffix word. Synthetic test: 5/5 fragments filtered, 4/4 real-shaped names preserved. **Patch is disk-only** — `scrapers/seven16-scraper/` is not a git repo. New post-demo carry-forward: clone the scrapers project out of OneDrive and `git init` (same fix applied to dotintel2 and saas-agency-database in earlier sessions).
...
- 51+ credential gate.
- Power-unit bands for trucking (already absorbed into Seven16 Intel Berkley mapping — see migration 0082 + the Berkley reference memory).
